...
//handles the inventory request
  static JSONObject inventory(JSONObject req) {
      ...
      JSONObject res = new JSONObject();
      // Placeholder implementation for inventory request
      res.put("ok", true);
      res.put("type", "inventory");
      res.put("items", new JSONArray()); // Placeholder for inventory items, to be filled with actual data
      return res;
  }

  // handles the roller request
  static JSONObject roller(JSONObject req) {
      System.out.println("Roller request: " + req.toString());
      JSONObject res = new JSONObject();
      try {
          int roll = req.getInt("roll");
          int sides = req.getInt("sides");

          if (roll < 1 || sides < 2) {
              res.put("ok", false);
              res.put("message", "Roll and sides must be positive integers");
          } else {
          ...
          res.put("ok", false);
          res.put("message", "Invalid request parameters");
      }
      return res;
  }
